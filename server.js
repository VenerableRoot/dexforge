...
// Tool Registry â€” available RE tools to bundle in output
const RE_TOOLS = '/opt/dexforge/re-tools';
const TOOL_REGISTRY = {
...
// Job page â€” serves the same UI but with jobId embedded
...
async function processFile(jobId, filePath, originalName, ext, jobDir, selectedTools = []) {
  ...
  // 1. Handle APKS â€” merge split APKs into a single unified APK using APKEditor
  ...
  // 3. SigTool + keytool â€” use base.apk for APKS (merged APK loses original signatures)
  ...
  // 6. Extract .so â€” from the (merged) APK which now contains all native libs
...
function buildReport(f, keytool) {
  ...
  let r = '';
  r += s + '\n';
  r += '  DEXFORGE â€” Signature & Certificate Report\n';
  r += '  ' + ts + '\n';
  r += s + '\n\n';
